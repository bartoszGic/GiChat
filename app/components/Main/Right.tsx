...
const Right = ({ isRightBarOpen }: RightProps) => {
	return (
		<>
			<section
				className={`absolute flex flex-col right-0 w-screen h-1/2 bg-slate-500 ease-in-out duration-200 transition-transform sm:h-1/3 sm:w-2/3 z-20 ${
					isRightBarOpen ? 'translate-y-0' : '-translate-y-full'
				}`}>
				<div className='grid px-4 py-6 w-full font-light text-xs sm:text-sm sm:h-1/3'>
